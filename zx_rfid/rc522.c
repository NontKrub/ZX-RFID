...
static esp_err_t rc522_write_n(uint8_t addr, uint8_t n, uint8_t *data) {
    ...
    esp_err_t ret = hndl->config->i2c->write(0, RC522_ADDR, buffer, n + 1);
...
static uint8_t* rc522_read_n(uint8_t addr, uint8_t n) {
    ...
    esp_err_t ret = hndl->config->i2c->read(0, RC522_ADDR, (uint8_t*)&addr, 1, buffer, n);
    assert(ret == ESP_OK);
...
esp_err_t rc522_init(rc522_config_t* config) {
    ...
    if(! (hndl = (rc522_handle_t)calloc(1, sizeof(struct rc522)))) {
        return ESP_ERR_NO_MEM;
    }

    if(! (hndl->config = (rc522_config_t*)calloc(1, sizeof(rc522_config_t)))) {
        rc522_destroy();
        return ESP_ERR_NO_MEM;
    }

    // copy config considering defaults
    hndl->config->i2c              = config->i2c;
...
static uint8_t* rc522_anticoll() {
    ...
    uint8_t buff[] = { 0x93, 0x20 };
    uint8_t* result = rc522_card_write(0x0C, buff, 2, &res_n);
